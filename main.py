...
class AssetManager:
    ...
    def generate_defaults(self):
        ...
        bg.fill((15, 15, 35)) # 深星空蓝
        for _ in range(100): # 绘制星星
...
class Game:
    ...
    def handle_shooting(self):
        # 射击改为使用记录的瞄准位置 last_aim_pos
        if pygame.mouse.get_pressed()[0] and self.player.weapon_cd <= 0:
            target = pygame.math.Vector2(self.last_aim_pos.x + camera.offset.x, self.last_aim_pos.y + camera.offset.y)
    ...
    def init_levelup(self):
        pygame.mouse.set_visible(True)
        self.levelup_input_active = False 
        options = [
        ...
        while True:
            ...
            # 3. 游戏运行
                ...
                for e in events:
                    ...
                    # 移动端修复：只有触控点在摇杆区域之外时，才更新瞄准坐标
                    if e.type in (pygame.MOUSEBUTTONDOWN, pygame.MOUSEMOTION):
                        dist = (pygame.math.Vector2(e.pos) - self.joystick.center).length()
                        if dist > self.joystick.radius + 50:
                            self.last_aim_pos = pygame.math.Vector2(e.pos)
                ...
                for enemy, bullets in hits.items():
                    for bullet in bullets:
                        ...
                        if enemy.hp <= 0:
                            ...
                            self.items.add(gem); self.all_sprites.add(gem)
                            enemy.kill(); break
                ...
                for r in range(-1, (HEIGHT // bg_h) + 2):
                    for c in range(-1, (WIDTH // bg_w) + 2):
                ...
                # 绘制准星：使用 last_aim_pos
                pygame.draw.circle(screen, GREEN, (int(self.last_aim_pos.x), int(self.last_aim_pos.y)), 5, 1)
                ...
                self.joystick.draw(screen); self.pause_icon_btn.draw(screen); self.draw_game_ui()
                pygame.display.flip()
                clock.tick(FPS)
            ...
            # 5. 升级
                ...
                overlay.set_alpha(200); overlay.fill(DARK_BG)
                ...
                for i, opt in enumerate(self.levelup_choices):
                    ...
                    pygame.draw.rect(screen, CYAN if is_hover else (100, 100, 100), rect, 3, border_radius=15)
                    t1 = font_card.render(f"{i+1}. {opt['name']}", True, YELLOW if is_hover else (200, 200, 200))
                    t2 = font_ui.render(opt["desc"], True, WHITE)
                    screen.blit(t1, (x+30, y+25)); screen.blit(t2, (x+30, y+70))

                if not self.levelup_input_active:
                    hint = font_ui.render("请松开鼠标/手指...", True, RED)
                    if not pygame.mouse.get_pressed()[0]: self.levelup_input_active = True
                else:
                    ...
                    if self.levelup_input_active:
                        ...
                        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                            for i, rect in enumerate(rects):
                                if rect.collidepoint(event.pos): idx = i
                        ...
                        if idx != -1:
                            ...
                            self.state = "RUNNING"; pygame.mouse.set_visible(False)
