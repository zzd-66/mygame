            ...
            # 3. 游戏运行
                ...
                for gem in hits:
                    ...
                    if self.player.check_levelup():
                        self.init_levelup() # 这里改用初始化
                        self.state = "LEVEL_UP"

                bg_img = assets.images['bg']
                bg_w, bg_h = bg_img.get_size()
                start_x, start_y = int(camera.offset.x % bg_w), int(camera.offset.y % bg_h)
